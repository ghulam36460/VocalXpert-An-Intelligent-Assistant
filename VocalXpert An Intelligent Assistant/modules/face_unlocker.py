...
def startDetecting():
	...
	required_recognitions = REQUIRED_RECOGNITIONS  # Configurable required recognitions
	...
	while attempt < max_attempts:
		...
		try:
			if len(face) == 0:
				# No face detected
				cv2.putText(image, "Looking for face...", (150, 450), 
				...
				cv2.putText(image, display_string, (100, 120), 
					cv2.FONT_HERSHEY_SIMPLEX, 1, (250, 120, 255), 2)
				...
				if confidence > PER_FRAME_THRESHOLD:  # Per-frame threshold (configurable)
					...
					cv2.putText(image, f"Unlocking... ({successful_recognitions}/{required_recognitions})", 
						(150, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
					
					if successful_recognitions >= required_recognitions:
						# Additional validation: require consistent high confidence
						if len(recent_confidences) >= 3:
							avg_confidence = sum(recent_confidences[-3:]) / 3  # Average of last 3
							# Use configurable average threshold
							if avg_confidence >= AVG_CONFIDENCE_THRESHOLD:
								logger.info(f"Face recognition successful - Average confidence: {avg_confidence:.1f}%, Samples: {len(recent_confidences)}")
								print(f"Face recognition successful - Average confidence: {avg_confidence:.1f}%")
								cv2.putText(image, "UNLOCKED!", (250, 400), 
								...
								# Reset counter if average confidence is not high enough
								successful_recognitions = max(0, successful_recognitions - 2)
								recent_confidences = recent_confidences[:-2]  # Remove last 2
								logger.warning(f"Face recognition failed - Low average confidence: {avg_confidence:.1f}%")
								print(f"Face recognition failed - Low average confidence: {avg_confidence:.1f}%")
								cv2.putText(image, "Verification failed - inconsistent", (100, 450), 
									cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
						else:
							# Not enough samples yet
							cv2.putText(image, "Gathering more samples...", (150, 480), 
								cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 165, 0), 2)
				else:
					successful_recognitions = max(0, successful_recognitions - 1)
					# Clear recent confidences if we get a low confidence
					if confidence < 60:
						recent_confidences.clear()
					
					if confidence > 50:
						cv2.putText(image, "Face detected but not recognized", (120, 450), 
							cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
					else:
						cv2.putText(image, "Face not recognized", (180, 450), 
							cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
				
				cv2.imshow('Face Recognition - VocalXpert', image)

		except Exception as e:
			cv2.putText(image, "Scanning...", (250, 450), 
	...
	logger.info(f"Face recognition session ended - Result: {result_text}, Attempts: {attempt}, Successful recognitions: {successful_recognitions}")
...
def retrain_model():
	...
	try:
		...
		# Get all face images
		face_files = [f for f in os.listdir(face_data_dir) if f.endswith('.jpg') or f.endswith('.png')]
		if not face_files:
		...
		if len(faces) < 5:
			logger.error(f"Insufficient training data: only {len(faces)} faces found. Need at least 5.")
			return False
