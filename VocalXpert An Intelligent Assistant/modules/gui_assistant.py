...
def play_sound(sound_name):
	...
	if SOUND_ENABLED and sound_name in SOUND_FILES:
		try:
			Thread(target=lambda: playsound.playsound(SOUND_FILES[sound_name]), daemon=True).start()
		except:
			pass
...
EXIT_COMMANDS = ['exit', 'quit', 'bye', 'goodbye', 'close', 'shut down', 'shutdown', 'stop', 'end']
...
def refresh_theme_widgets():
	...
	if UI_ELEMENTS.get('header_frame') is not None:
		...
		cfg(UI_ELEMENTS.get('app_name_lbl'), bg=theme['header_bg'], fg=textColor)
		cfg(UI_ELEMENTS.get('ai_status_frame'), bg=theme['header_bg'])
		cfg(UI_ELEMENTS.get('ai_dot'), bg=theme['header_bg'])
		cfg(UI_ELEMENTS.get('ai_status_text'), bg=theme['header_bg'], fg=TEXT_MUTED)
	...
	if UI_ELEMENTS.get('bottomFrame1') is not None:
		...
		if cbl_widget is not None:
			image_ref = globals().get('cblDarkImg') if KCS_IMG == 1 else globals().get('cblLightImg')
		...
		if setting_widget is not None:
			setting_image = globals().get('sphDark') if KCS_IMG == 1 else globals().get('sphLight')
			setting_widget.configure(bg=SURFACE_DARK, activebackground=SURFACE_LIGHT)
		...
		if kb_widget is not None:
			kb_image = globals().get('kbphDark') if KCS_IMG == 1 else globals().get('kbphLight')
			kb_widget.configure(bg=SURFACE_DARK, activebackground=SURFACE_LIGHT)
			if kb_image is not None:
				kb_widget.configure(image=kb_image)
				kb_widget.image = kb_image
		cfg(UI_ELEMENTS.get('micBtn'), bg=theme['mic_btn_bg'], activebackground=SURFACE_LIGHT)
		cfg(UI_ELEMENTS.get('AITaskStatusLbl'), bg=AITaskStatusLblBG, fg=textColor)
	if UI_ELEMENTS.get('input_container') is not None:
		...
		if UI_ELEMENTS.get('UserField') is not None:
			UI_ELEMENTS['UserField'].configure(bg=SURFACE_LIGHT, insertbackground=ACCENT_PRIMARY)
			if placeholder_active:
				UI_ELEMENTS['UserField'].configure(fg=theme['placeholder_fg'])
			else:
				UI_ELEMENTS['UserField'].configure(fg=textColor)
		cfg(UI_ELEMENTS.get('sendBtn'), bg=theme['send_btn_bg'], fg=theme['send_btn_fg'], activebackground=BTN_PRIMARY_HOVER, activeforeground='white')
	if UI_ELEMENTS.get('settingsFrame') is not None:
		settings_widgets = ['settingsFrame', 'settingsLbl', 'userPhoto', 'userName', 'assLbl', 'voiceRateLbl', 'volumeLbl', 'themeLbl', 'chooseChatLbl']
		for key in settings_widgets:
			widget = UI_ELEMENTS.get(key)
			if widget is not None:
				fg = textColor if key in {'settingsLbl', 'userName', 'assLbl', 'voiceRateLbl', 'volumeLbl', 'themeLbl', 'chooseChatLbl'} else None
				cfg(widget, bg=background, fg=fg)
		cfg(UI_ELEMENTS.get('volumeBar'), bg=background, fg=textColor, highlightbackground=background)
...
def speak(text, display=False, icon=False):
	...
	def show_bot_message():
		...
		if chat:
			if icon:
				Label(chat, image=botIcon, bg=chatBgColor).pack(anchor='w', pady=0)
			if display:
...
def record(clearChat=True, iconDisplay=True):
	...
	try:
		with sr.Microphone() as source:
			...
			if iconDisplay and UI_ELEMENTS.get('chat_frame'):
				def show_user_message():
					chat = UI_ELEMENTS.get('chat_frame')
					if chat:
						Label(chat, image=userIcon, bg=chatBgColor).pack(anchor='e', pady=0)
						attachTOframe(said)
		...
		safe_ui_update(lambda: speak('Your system appears to be offline...', True, True))
		return 'None'
...
def voiceMedium():
	...
	while True:
		try:
			...
			Thread(
				target=lambda q=query: process_input(q.lower()), 
				daemon=True
			).start()
...
def keyboardInput(event=None):
	...
	if UI_ELEMENTS.get('chat_frame') is not None:
		Label(UI_ELEMENTS['chat_frame'], image=userIcon, bg=chatBgColor).pack(anchor='e', pady=0)
...
def main(text):
	...
	try:
		...
		if isContain(text, ['youtube','video']):
			speak("Ok "+ownerDesignation+", here is a video for you...", True, True)
			try:
		...
		if isContain(text, ['map', 'direction']):
			if "direction" in text:
				...
				try:
					...
					speak("I think the location is not proper, try again!", True, True)
			else:
		...
		if isContain(text, ['open','launch','type','save','delete','select','press enter','open site', 'start', 'run']):
...
def build_feature_panel(parent):
	...
	for idx, item in enumerate(FEATURE_ITEMS):
		card = Frame(feature_panel, bg=SURFACE_DARK, highlightbackground='#1f2937', highlightthickness=1, bd=0, cursor='hand2')
		card.grid(row=idx//cols, column=idx%cols, padx=6, pady=6, sticky='nsew')
		card.columnconfigure(0, weight=1)
		icon_lbl = Label(card, text=item['icon'], font=('Segoe UI Emoji', 20), bg=SURFACE_DARK, fg=textColor)
		icon_lbl.pack(pady=(10, 2))
		title_lbl = Label(card, text=item['title'], font=('Segoe UI', 12, 'bold'), bg=SURFACE_DARK, fg=textColor)
		title_lbl.pack()
		desc_lbl = Label(card, text=item['description'], font=('Segoe UI', 10), bg=SURFACE_DARK, fg=TEXT_MUTED, wraplength=150, justify='center')
		desc_lbl.pack(padx=12, pady=(2, 12))
		for widget in (card, icon_lbl, title_lbl, desc_lbl):
			widget.bind('<Button-1>', lambda e, cmd=item['command']: feature_card_click(cmd))
...
def attachTOframe(text,bot=False):
	...
	if bot:
		...
		botchat = Label(msg_frame, text=text, bg=botChatTextBg, fg=botChatText, justify=LEFT, 
						wraplength=280, font=('Segoe UI', 11), padx=14, pady=10)
		botchat.pack()
		
		# Add timestamp
		timestamp = Label(msg_container, text=strftime('%I:%M %p'), bg=chatBgColor, 
...
def showSingleImage(type, data=None):
	...
	try:
		img0 = ImageTk.PhotoImage(Image.open('Downloads/0.jpg').resize((90,110), RESAMPLE_MODE))
	except:
		pass
	img1 = ImageTk.PhotoImage(Image.open('assets/images/heads.jpg').resize((220,200), RESAMPLE_MODE))
	img2 = ImageTk.PhotoImage(Image.open('assets/images/tails.jpg').resize((220,200), RESAMPLE_MODE))
	...
	if type=="weather":
		...
		img3 = ImageTk.PhotoImage(Image.open('assets/images/dice/'+type+'.jpg').resize((200,200), RESAMPLE_MODE))
...
def showImages(query):
	...
	#loading images
	img0 = ImageTk.PhotoImage(Image.open('Downloads/0.jpg').resize((w,h), RESAMPLE_MODE))
	img1 = ImageTk.PhotoImage(Image.open('Downloads/1.jpg').resize((w,h), RESAMPLE_MODE))
	img2 = ImageTk.PhotoImage(Image.open('Downloads/2.jpg').resize((w,h), RESAMPLE_MODE))
	img3 = ImageTk.PhotoImage(Image.open('Downloads/3.jpg').resize((w,h), RESAMPLE_MODE))
	#Displaying
...
def UpdateIMAGE():
	...
	userProfileImg = ImageTk.PhotoImage(Image.open("assets/images/avatars/a"+str(ownerPhoto)+".png").resize((120, 120), RESAMPLE_MODE))
...
def progressbar():
	...
	progress_bar = ttk.Progressbar(splash_root, style="modern.Horizontal.TProgressbar", 
								  orient="horizontal", mode="determinate", length=350)
...
if __name__ == '__main__':
	...
	# Logo/Icon area
	Label(gradient_frame, text="🎙️", font=('Segoe UI Emoji', 40), bg='#0f172a').pack(pady=(25, 5))
	
	# App title with modern styling
	title_label = Label(gradient_frame, text="VocalXpert", font=('Segoe UI', 32, 'bold'), bg='#0f172a', fg='#6366f1')
	title_label.pack(pady=(5, 2))
	
	subtitle_label = Label(gradient_frame, text="Your Intelligent AI Assistant", font=('Segoe UI', 12), bg='#0f172a', fg='#94a3b8')
	subtitle_label.pack(pady=(0, 15))
	
	splash_label = Label(gradient_frame, text="Starting...", font=('Segoe UI', 10), bg='#0f172a', fg='#64748b')
	...
	# App name in header
	app_name_lbl = Label(header_frame, text="🎙️ VocalXpert", font=('Segoe UI', 15, 'bold'), 
				bg=ACTIVE_THEME['header_bg'], fg=textColor)
	...
	ai_dot = Label(ai_status_frame, text="●", font=('Segoe UI', 8), bg=ACTIVE_THEME['header_bg'], fg='#f59e0b')
	ai_dot.pack(side=LEFT)
	ai_status_text = Label(ai_status_frame, text=" Offline", font=('Segoe UI', 9), 
					  bg=ACTIVE_THEME['header_bg'], fg=TEXT_MUTED)
	...
	def quick_apps():
		Label(chat_frame, image=botIcon, bg=chatBgColor).pack(anchor='w', pady=0)
		attachTOframe("Which app would you like to open? (e.g., 'open chrome', 'open notepad', 'open calculator')", True)
	...
	qa_canvas = Canvas(quick_actions_frame, bg=ACTIVE_THEME['qa_bg'], height=40, highlightthickness=0)
	...
	chat_canvas = Canvas(chat_container, bg=chatBgColor, highlightthickness=0, width=385, height=480)
	chat_scrollbar = Scrollbar(chat_container, orient="vertical", command=chat_canvas.yview, 
							  bg='#334155', troughcolor=chatBgColor)
	...
	cbl = Label(VoiceModeFrame, fg='white', image=cblDarkImg if KCS_IMG==1 else cblLightImg, bg=SURFACE_DARK)
	cbl.pack(pady=(14, 6))
	AITaskStatusLbl = Label(VoiceModeFrame, text='    Offline', fg=textColor, bg=AITaskStatusLblBG, font=('Segoe UI', 13, 'bold'))
	...
	userProfileImg = ImageTk.PhotoImage(userProfileImg.resize((120, 120), RESAMPLE_MODE))
	...
	#Change Photo
	chngPh = ImageTk.PhotoImage(Image.open("assets/images/avatars/changephoto2.png").resize((120, 120), RESAMPLE_MODE))
