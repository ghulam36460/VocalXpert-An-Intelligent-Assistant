...
def wikiResult(query):
    ...
    try:
        ...
        # Clean the query
        query = query.replace('wikipedia', '').replace('search', '').replace('wiki', '').strip()
        ...
        try:
            ...
            # Set rate limiting
            wikipedia.set_rate_limiting(True, min_wait=datetime.timedelta(0, 0, 100000))
            ...
            try:
                ...
                try:
                    ...
                    return f"Wikipedia: {summary[:200]}... (Opening full page in browser)"
                except:
...
class WEATHER:
		...
		try:
			...
			if WEATHER_API_KEY:
				...
				if api_res.status_code == 200:
					...
					self.currCondition = weather_data['weather'][0]['description'].title()
					return
	...
	def weather(self):
		...
		if not self.tempValue:
			self.updateWeather()
		self.speakResult = "Currently in " + self.city + ", its " + self.tempValue + " degrees, with " + self.currCondition 
...
def email(rec_email=None, text="Hello, It's VocalXpert here...", sub='VocalXpert'):
    ...
    try:
        ...
        for g in soup.find_all('div', class_='g'):
            ...
            if a and a.has_attr('href'):
                link = a['href']
            snippet_el = g.find('span', class_='st') or g.find('div', class_='IsZvec')
            if title_el:
        ...
        logger.info(f"No structured results found for query: {query}. Opening browser search.")
        try:
            webbrowser.open(url)
        except Exception:
            logger.debug("Could not open browser for search URL")
