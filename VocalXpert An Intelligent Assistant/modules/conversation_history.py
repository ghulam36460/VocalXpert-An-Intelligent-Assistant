...
class ConversationHistory:
    ...
    def _load_history(self) -> Dict:
        ...
        try:
            ...
            logger.warning(
                "Conversation history file corrupted or missing, resetting to default"
            )
            default_history = {
                "conversations": [],
                "metadata": {
    ...
    def _save_history(self):
        ...
        self.history["metadata"]["last_updated"] = datetime.datetime.now(
        ).isoformat()
        with open(self.history_file, "w", encoding="utf-8") as f:
    ...
    def _generate_summary(self, user_query: str, ai_response: str) -> str:
        ...
        conversation_type = self._classify_conversation(
    ...
    def add_conversation(self,
        ...
        conversation = {
            "id":
                len(self.history["conversations"]) + 1,
            "timestamp":
            ...
            "source":
                source,
            "conversation_type":
                self._classify_conversation(user_query, ai_response),
        }

        # Add to history
        self.history["conversations"].append(conversation)
        self.history["metadata"]["total_conversations"] = len(
    ...
    def get_conversations_by_type(self,
                                  conv_type: str,
    ...
    def get_conversation_analytics(self) -> Dict:
        ...
        # Count by type
        types = Counter(
            conv.get("conversation_type", "unknown") for conv in conversations)

        # Count by source
        sources = Counter(
            conv.get("source", "unknown") for conv in conversations)
        ...
        all_queries = " ".join(
            conv["user_query"] for conv in conversations[-100:])  # Last 100
        ...
        return {
            ...
            "daily_activity":
                dict(daily_counts),
            "common_topics": [
                word for word, count in common_words if count > 1
            ],
        }
    ...
    def get_personalization_data(self) -> Dict:
        ...
        preferences = {
            "preferred_topics":
                analytics.get("common_topics", [])[:5],
            "interaction_style":
                ("command_heavy" if analytics.get("conversation_types", {}).get(
                    "command", 0) > 50 else "conversational"),
            "activity_level":
                ("high" if analytics.get("total_conversations", 0) > 100 else
                 ("moderate"
                  if analytics.get("total_conversations", 0) > 20 else "low")),
        }
...
def view_history():
    ...
    print(
        f"Total conversations: {len(conversation_manager.history['conversations'])}"
    )
    print("-" * 50)
    ...
    for conv in conversations:
        ...
        print(
            f"AI: {conv['ai_response'][:100]}{'...' if len(conv['ai_response']) > 100 else ''}"
        )
...
def get_user_insights():
    ...
    print(
        f"Activity level: {conversation_manager.get_personalization_data()['preferences']['activity_level']}"
    )
