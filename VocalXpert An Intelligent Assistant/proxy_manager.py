...
try:
    ...
    BeautifulSoup = None
    print(
        "Warning: BeautifulSoup not available. Install with: pip install beautifulsoup4"
    )
...
try:
    ...
    print(
        "Warning: free-proxy not available. Install with: pip install free-proxy"
    )
try:
    ...
    print(
        "Warning: requests-ip-rotator not available. Install with: pip install requests-ip-rotator"
    )
try:
    ...
    print(
        "Warning: proxyscrape not available. Install with: pip install proxyscrape"
    )
try:
    import socks
    import socket
...
class ProxySourceManager:
    ...
    async def fetch_from_all_sources(self,
        ...
        for source_name, source_func in self.sources.items():
            try:
                ...
                proxies = await self._fetch_from_source(source_name,
                                                        source_func,
                                                        limit_per_source)
                all_proxies.extend(proxies)
                self.stats[f"{source_name}_success"] += len(proxies)
                logger.info(
                    f"Fetched {len(proxies)} proxies from {source_name}")
    ...
    async def _fetch_from_source(self, source_name: str, source_func,
                                 limit: int) -> List[ProxyInfo]:
...
class FreeProxyGenerator:
    ...
    def _setup_sources(self):
        ...
        if FREE_PROXY_AVAILABLE:
            self.source_manager.add_source("free_proxy_http",
                                           self._fetch_free_proxy_http)
            self.source_manager.add_source("free_proxy_https",
                                           self._fetch_free_proxy_https)
            self.source_manager.add_source("free_proxy_elite",
                                           self._fetch_free_proxy_elite)

        # Manual API sources
        self.source_manager.add_source("proxy_list_api",
                                       self._fetch_proxy_list_api)
        # Removed gimmeproxy API as it's not working
        self.source_manager.add_source("proxyrotator_api",
                                       self._fetch_proxyrotator_api)

        # ProxyScrape sources
        if PROXYSCRAPE_AVAILABLE:
            self.source_manager.add_source("proxyscrape_http",
                                           self._fetch_proxyscrape_http)
            self.source_manager.add_source("proxyscrape_https",
                                           self._fetch_proxyscrape_https)
            self.source_manager.add_source("proxyscrape_socks4",
                                           self._fetch_proxyscrape_socks4)
            self.source_manager.add_source("proxyscrape_socks5",
                                           self._fetch_proxyscrape_socks5)

        # Web scraping sources
        self.source_manager.add_source("free_proxy_list",
                                       self._fetch_free_proxy_list)
        self.source_manager.add_source("static_proxies",
                                       self._fetch_static_proxies)

        # Reliable proxy sources
        self.source_manager.add_source("free_proxy_list_net",
                            ...
                            if ":" in line and len(line.split(":")) == 2:
                                ...
                                try:
                                    ...
                                    if 1 <= port <= 65535:  # Valid port range
                                        protocol = ("https" if "https"
                                                    in endpoint else "http")
                                        proxies.append(
                                            ProxyInfo(
                                                host=host,
                                                port=port,
        ...
        try:
            ...
            if proxy_list and len(proxy_list) > 0:
                for proxy in proxy_list[:limit]:
                    if proxy and hasattr(proxy, "host") and hasattr(
                            proxy, "port"):
                        proxies.append(
                            ProxyInfo(
    ...
    def _fetch_free_proxy_list_net(self, limit: int = 10) -> List[ProxyInfo]:
        ...
        try:
            if not BS4_AVAILABLE or BeautifulSoup is None:
                logger.warning(
                    "BeautifulSoup not available for free-proxy-list.net scraping"
                )
                return proxies
            ...
            if response.status_code == 200:
                ...
                rows = tbody.find_all("tr") if tbody else table.find_all(
                    "tr")[1:]
                for row in rows[:limit]:
                    ...
                    if len(cols) >= 7:
                        ...
                        country = (cols[3].text.strip()
                                   if len(cols) > 3 and cols[3] else
                                   (cols[2].text.strip()
                                    if len(cols) > 2 and cols[2] else ""))
                        anonymity = (cols[4].text.strip()
                                     if len(cols) > 4 and cols[4] else "")
                        https = (cols[6].text.strip().lower() == "yes"
                                 if len(cols) > 6 and cols[6] else False)
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        rows = (tbody.find_all("tr")
                                if tbody else table.find_all("tr")[1:])
                        for row in rows:
                            if row is None:
                                continue
                            ...
                            if len(cols) >= 7:
                                ...
                                country = (cols[3].text.strip()
                                           if len(cols) > 3 and cols[3] else
                                           (cols[2].text.strip() if
                                            len(cols) > 2 and cols[2] else ""))
                                anonymity = (cols[4].text.strip() if
                                             len(cols) > 4 and cols[4] else "")
                                https = (cols[6].text.strip().lower() == "yes"
                                         if len(cols) > 6 and cols[6] else
                                         False)
    ...
    def _parse_proxy_url(
            self, proxy_url: str) -> Tuple[Optional[str], Optional[int]]:
    ...
    async def generate_proxies(self, count: int = 50) -> List[ProxyInfo]:
        ...
        proxies = await self.source_manager.fetch_from_all_sources(
            count // len(self.source_manager.sources))
...
class ProxyValidator:
    ...
    async def validate_proxies(self,
                               ...
                               max_workers: int = 20) -> List[ProxyInfo]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_proxy):
                ...
                try:
                    ...
                    logger.error(
                        f"Validation failed for {proxy.host}:{proxy.port}: {e}")

        logger.info(
            f"Validated {len(validated_proxies)} working proxies out of {len(proxies)}"
        )
            ...
            for test_url in self.test_urls:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.debug(
                            f"Proxy {proxy.host}:{proxy.port} validated with {test_url} in {response_time:.2f}s"
                        )
                        return proxy

                except Exception as e:
                    logger.debug(
                        f"Proxy {proxy.host}:{proxy.port} failed {test_url}: {e}"
                    )
                    continue  # Try next URL

            # If all URLs failed, proxy is not working
            logger.debug(
                f"Proxy {proxy.host}:{proxy.port} failed all test URLs")

        except Exception as e:
            logger.debug(
                f"Proxy {proxy.host}:{proxy.port} validation error: {e}")
...
class AdvancedProxyManager:
    ...
    def __init__(self, options: Dict[str, Any] = None):
        ...
        self.validation_interval = self.options.get("validation_interval",
                                                    300)  # 5 minutes
        self.rotation_strategy = self.options.get("rotation_strategy",
                                                  "round_robin")
    ...
    async def initialize(self):
        ...
        logger.info(
            f"Proxy manager initialized with {len(self.working_proxies)} working proxies"
        )
    ...
    async def refresh_proxies(self):
        ...
        logger.info(
            f"Refreshed proxies: {len(validated_proxies)} working out of {len(new_proxies)} total"
        )
    ...
    def get_multiple_proxies(self, count: int) -> List[ProxyInfo]:
        ...
        with self.lock:
            ...
            if self.rotation_strategy == "random":
                ...
                self.current_index = (self.current_index + count) % len(
                    self.working_proxies)
                return proxies

    def mark_proxy_result(self,
                          proxy: ProxyInfo,
                          success: bool,
            ...
            if proxy.success_rate < 20.0:
                with self.lock:
                    if proxy in self.working_proxies:
                        ...
                        logger.info(
                            f"Removed failing proxy {proxy.host}:{proxy.port}")
    ...
    def export_proxies(self, format: str = "json") -> str:
        ...
        with self.lock:
            if format == "json":
                return json.dumps(
                    [proxy.to_dict() for proxy in self.working_proxies],
                    indent=2)
...
async def test_proxy_manager():
    ...
    for i in range(5):
        ...
        if proxy:
            print(
                f" {i+1}. {proxy.host}:{proxy.port} (speed: {proxy.speed:.2f}s)"
            )
        else:
    ...
    if args.generate:
        ...
        async def generate_and_output():
            ...
            result = {
                ...
                "working_count":
                    len(validated),
                "success_rate":
                    (len(validated) / len(proxies) * 100) if proxies else 0,
            ...
            if args.output:
                with open(args.output, "w") as f:
                    f.write(output)
                print(
                    f"Exported {len(validated)} proxies to {args.output}",
                    file=sys.stderr,
                )
            else:
                print(output)
        ...
        async def validate_from_file():
            try:
                ...
                print(
                    f"Validating {len(proxies)} proxies from {args.validate}...",
                    file=sys.stderr,
                )
