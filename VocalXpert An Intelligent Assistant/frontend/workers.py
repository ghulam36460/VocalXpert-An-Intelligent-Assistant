...
class VoiceRecognitionWorker(QThread):
    ...
    def _init_vosk(self):
        ...
        try:
            ...
            # Make stream non-blocking for faster shutdown
            # Note: This might cause issues with some audio devices
            try:
                self._audio_stream.stop_stream()  # Stop initially
            except:
                pass
    ...
    def run(self):
        ...
        if not VOSK_AVAILABLE:
            ...
            self.status_changed.emit("❌ Offline speech recognition not available")
            self.error_occurred.emit("Vosk library not installed")
            return
        
        if not self._init_vosk():
            self.status_changed.emit("❌ Could not initialize speech recognition")
        ...
        try:
            ...
            while self._is_running and (time.time() - start_time) < self.timeout:
                if not self._is_running:
                    break  # Check stop flag before each read
    ...
    def run(self):
        ...
        try:
            ...
            if response and response != "None":
                is_ai = normal_chat.is_ai_online() if hasattr(normal_chat, 'is_ai_online') else False
                self.response_ready.emit(response, is_ai)
                self.status_changed.emit("✓ Ready")
            else:
                self.response_ready.emit("I'm not sure how to respond to that.", False)
                self.status_changed.emit("✓ Ready")
...
class CommandWorker(QThread):
        ...
        try:
            ...
            if ai_chat.is_online():
                ...
                if is_ai and ai_response:
                    ...
                    import re
                    command_match = re.search(r'\[COMMAND:([^]]+)\](.*?)\[/COMMAND\]', ai_response, re.IGNORECASE)
                    ...
                    if command_match:
                        ...
                        return self._execute_ai_command(action, params, ai_response)
        ...
        try:
            ...
            # Web & Search
                ...
                if not self.settings.get('web_scraping_enabled', True):
                    ...
                    try:
                        ...
                        self.result_ready.emit(reply if reply else "I'm not sure.", 'chat')
                        return True
                    except Exception:
                        self.result_ready.emit("Web scraping mode is disabled. Enable it in Settings > Web Features.", 'error')
                        return True
                from modules import web_scrapping
                try:
                    results = web_scrapping.enhanced_google_search(params, num_results=5)
                    ...
                    try:
                        ...
                        pass
                    self.result_ready.emit("Opening search results...", 'search')
                return True
                ...
                if not self.settings.get('web_scraping_enabled', True):
                    try:
                        from modules import normal_chat
                        reply = normal_chat.reply(params)
                        self.result_ready.emit(reply if reply else "I couldn't fetch Wikipedia results right now.", 'wiki')
                        return True
                    except Exception:
                        self.result_ready.emit("Web scraping mode is disabled. Enable it in Settings > Web Features.", 'error')
                        return True
                ...
                if not self.settings.get('web_scraping_enabled', True):
                    try:
                        from modules import normal_chat
                        reply = normal_chat.reply(params)
                        self.result_ready.emit(reply if reply else "I can't access YouTube right now.", 'youtube')
                        return True
                    except Exception:
                        self.result_ready.emit("Web scraping mode is disabled. Enable it in Settings > Web Features.", 'error')
                        return True
                from modules import web_scrapping
                video_name = params.replace('play ', '').replace('on youtube', '').strip()
                web_scrapping.playonYT(video_name)
                self.result_ready.emit(f"Playing {video_name} on YouTube...", 'youtube')
                return True
                ...
                if not self.settings.get('web_scraping_enabled', True):
                    try:
                        from modules import normal_chat
                        reply = normal_chat.reply(params or 'what is the weather')
                        self.result_ready.emit(reply if reply else "I can't fetch weather right now.", 'weather')
                        return True
                    except Exception:
                        self.result_ready.emit("Web scraping mode is disabled. Enable it in Settings > Web Features.", 'error')
                        return True
                ...
                if not self.settings.get('web_scraping_enabled', True):
                    try:
                        ...
                        self.result_ready.emit(reply if reply else "I can't fetch news right now.", 'news')
                        return True
                    except Exception:
                        self.result_ready.emit("Web scraping mode is disabled. Enable it in Settings > Web Features.", 'error')
                        return True
                ...
                self.result_ready.emit("Opening directions in Google Maps...", 'maps')
                return True
            ...
            # Productivity
                ...
                if action == 'todo_add':
                    ...
                    if todos:
                        ...
                        self.result_ready.emit("Your to-do list is empty!", 'todo')
                else:
            ...
            # Games
                ...
                if action == 'dice':
                    ...
                    self.result_ready.emit("Opening Rock Paper Scissors!", 'game')
                    return True
            ...
            # Information
                ...
                dt = DateTime()
                self.result_ready.emit(f"The current time is {dt.currentTime()}", 'time')
                return True
                ...
                response = f"{result[0]}\n{result[1]}" if isinstance(result, list) else str(result)
                self.result_ready.emit(response, 'dictionary')
                return True
            
            elif action == 'translate':
                from modules import normal_chat
                result = normal_chat.lang_translate(params, "es")  # Default to Spanish
                self.result_ready.emit(str(result), 'translate')
                return True
            
            # If action not recognized, fall back to AI response
            self.result_ready.emit(original_response.replace(f'[COMMAND:{action}]{params}[/COMMAND]', '').strip(), 'ai_chat')
            return True
    ...
    def run(self):
        ...
        try:
            ...
            # Check for explicit web scraper commands first (before AI parsing)
            cmd_lower = self.command.lower()
            if SCRAPER_AVAILABLE and (cmd_lower.startswith('web scrapper') or cmd_lower.startswith('web scraper')):
                try:
                    from modules import normal_chat
                    result = normal_chat.handle_web_scraping_command(self.command)
                    self.result_ready.emit(result, 'web_scraping')
                    self.status_changed.emit("✓ Ready")
                    return
                except Exception as e:
                    self.result_ready.emit(f"Web scraping failed: {str(e)}", 'error')
                    self.status_changed.emit("✓ Ready")
                    return
            
            # First, try intelligent AI-based command parsing
            ...
            if 'weather' in cmd:
                try:
                    ...
                    self.result_ready.emit(f"Could not fetch weather: {str(e)[:30]}", 'error')
                return
            ...
            if any(word in cmd for word in ['game', 'play']):
                if any(word in cmd for word in ['rock', 'paper', 'scissor', 'rps']):
                    self.open_game.emit('rps')
                    self.result_ready.emit("Opening Rock Paper Scissors!", 'game')
                    ...
                    self.result_ready.emit("Available games: Rock Paper Scissors, Dice Roll, Coin Flip", 'game')
                return
            ...
            if 'search' in cmd:
                ...
                from modules import web_scrapping
                try:
                    results = web_scrapping.enhanced_google_search(cmd, num_results=5)
                    if isinstance(results, list):
                        lines = []
                        for i, r in enumerate(results[:5]):
                            title = r.get('title', 'No title')
            ...
            # Calculator / Math
            if any(word in cmd for word in ['calculate', 'math', 'plus', 'minus', 'multiply', 'divide']):
                from modules import math_function
            ...
            if any(word in cmd for word in ['open', 'launch', 'start']):
                ...
                try:
                    ...
                    if write_match:
                        # Small delay might be necessary in real runs; we just call helper
                        resp = app_control.write_text(write_text)
                        self.result_ready.emit(f"Opened {app_name} and typed your text.", 'system')
                    else:
            ...
            if 'dictionary' in cmd or 'meaning of' in cmd or 'definition' in cmd:
                try:
                    ...
                    self.result_ready.emit(f"Could not get definition: {str(e)[:30]}", 'error')
                return
            ...
            if 'create file' in cmd or 'new file' in cmd or 'make file' in cmd:
                try:
                    ...
                    self.result_ready.emit(f"Could not create file: {str(e)[:30]}", 'error')
                return
            ...
            if 'create project' in cmd or 'html project' in cmd or 'web project' in cmd:
                try:
                    ...
                    self.result_ready.emit(f"Could not create project: {str(e)[:30]}", 'error')
                return
            ...
            if 'translate' in cmd:
                try:
                    ...
                    result = normal_chat.lang_translate(cmd, "es")  # Default to Spanish
                    self.result_ready.emit(str(result), 'translate')
                except Exception as e:
                    self.result_ready.emit(f"Translation failed: {str(e)[:30]}", 'error')
                return
            ...
            if 'map' in cmd or 'direction' in cmd:
                try:
                    ...
                    if 'direction' in cmd:
                        ...
                        web_scrapping.giveDirections("current location", cmd.replace('direction', '').replace('to', '').strip())
                        self.result_ready.emit("Opening directions in Google Maps...", 'maps')
                    else:
                        web_scrapping.maps(cmd)
                        self.result_ready.emit("Opening Google Maps...", 'maps')
                except Exception as e:
                    self.result_ready.emit(f"Could not open maps: {str(e)[:30]}", 'error')
                return
            ...
            if 'joke' in cmd:
                try:
                    ...
                    self.result_ready.emit("Why did the programmer quit? Because he didn't get arrays! 😄", 'joke')
                return
            ...
            if 'youtube' in cmd or 'play video' in cmd:
                try:
                    from modules import web_scrapping
                    video_name = cmd.replace('youtube', '').replace('play video', '').replace('play', '').strip()
                    web_scrapping.playonYT(video_name)
                    self.result_ready.emit(f"Playing {video_name} on YouTube...", 'youtube')
                except Exception as e:
                    self.result_ready.emit(f"Could not open YouTube: {str(e)[:30]}", 'error')
                return
            ...
            if 'timer' in cmd or 'remind' in cmd or 'alarm' in cmd:
                try:
                    from modules import app_timer
                    # startTimer expects a query string like "set timer 5 minutes"
                    result = app_timer.startTimer(cmd)
                    self.result_ready.emit(result, 'timer')
                except Exception as e:
                    self.result_ready.emit(f"Could not set timer: {str(e)[:50]}", 'error')
                return
            ...
            if 'todo' in cmd or 'task' in cmd:
                try:
                    ...
                    if any(word in cmd for word in ['add', 'create', 'new']):
                        ...
                        result = todo_handler.toDoList(task_text if task_text else "New task")
                        self.result_ready.emit(result, 'todo')
                    elif any(word in cmd for word in ['show', 'list', 'view', 'get']):
                        todos = todo_handler.showtoDoList()
                        if todos:
                            todo_text = "\n".join([f"• {t}" for t in todos])
                            self.result_ready.emit(todo_text, 'todo')
                            ...
                            self.result_ready.emit("Your to-do list is empty!", 'todo')
                    elif any(word in cmd for word in ['delete', 'remove', 'done', 'complete', 'clear']):
                        if 'clear' in cmd or 'all' in cmd:
                            ...
                            remove_text = cmd
                            for word in ['delete', 'remove', 'done', 'complete', 'todo', 'task', 'from list']:
                                remove_text = remove_text.replace(word, '').strip()
                    ...
                    self.result_ready.emit(f"Todo error: {str(e)[:50]}", 'error')
                return
            ...
            if 'email' in cmd or 'mail' in cmd:
                try:
                    ...
                    self.result_ready.emit(f"Could not open email: {str(e)[:30]}", 'error')
                return
            
            # Website opening
            if 'website' in cmd or 'open' in cmd and any(site in cmd for site in ['google', 'youtube', 'facebook', 'twitter', 'github']):
                try:
                    from modules import app_control
                    app_control.open_website(cmd)
                    self.result_ready.emit("Opening website...", 'web')
                except Exception as e:
                    self.result_ready.emit(f"Could not open website: {str(e)[:30]}", 'error')
                return
            ...
            if response and response != "None":
                self.result_ready.emit(response, 'chat')
            else:
                self.result_ready.emit("I'm not sure how to help with that. Try asking me about weather, time, or to play a game!", 'chat')
...
class ContinuousVoiceWorker(QObject):
        ...
        if self._current_worker:
            ...
            if self._current_worker.isRunning():
                if not self._current_worker.wait(2000):  # Wait up to 2 seconds
                    logger.warning("Voice recognition worker didn't stop gracefully")
    ...
    def on_tts_finished(self):
        ...
        self._waiting_for_tts = False
        if self._is_active:
            QTimer.singleShot(500, self._start_recognition)  # Small delay after TTS
    ...
    def _on_error(self, error: str):
        ...
        if self._is_active and error == "timeout":
