...
from PySide6.QtWidgets import (
    ...
    QLabel, QFrame, QPushButton, QLineEdit, QStackedWidget,
...
class FaceRecognitionWorker(QThread):
    ...
    status_changed = Signal(str, str)  # (message, status_type: info/success/error/warning)
    progress = Signal(int)
    ...
    def run(self):
        ...
        try:
            ...
            if result:
                ...
                user = UserData()
                user.extractData()
                user_name = user.getName().split()[0] if user.getName() != 'None' else "User"
                
                self.status_changed.emit(f"✅ Welcome back, {user_name}!", "success")
...
class FaceRegistrationDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        name_label.setFont(QFont(FONTS["family"], FONTS["size_md"], FONTS["weight_medium"]))
        ...
        self.start_btn.setFont(QFont(FONTS["family"], FONTS["size_md"], FONTS["weight_medium"]))
        
        self.cancel_btn = QPushButton("Cancel")
        self.cancel_btn.clicked.connect(self.reject)
        self.cancel_btn.setFont(QFont(FONTS["family"], FONTS["size_md"], FONTS["weight_medium"]))
    ...
    def _load_face_detector(self):
        ...
        cascade_path = Path(__file__).parent.parent / "Cascade" / "haarcascade_frontalface_default.xml"
    ...
    def _start_registration(self):
        ...
        try:
            ...
            self.face_samples = []
            self.progress_bar.setValue(0)
            self.status_label.setText("📸 Position your face in the camera and hold still...")
    ...
    def _update_camera(self):
        ...
        try:
            ...
            if self.face_classifier is not None:
                ...
                for (x, y, w, h) in faces:
                    ...
                    if len(self.face_samples) < self.max_samples:
                        ...
                        # Update progress
                        progress = int((len(self.face_samples) / self.max_samples) * 100)
                        self.progress_bar.setValue(progress)
                        
                        if len(self.face_samples) >= self.max_samples:
                            self.status_label.setText("✅ Enough samples collected! Click 'Stop & Train'")
                            break
            ...
            if len(self.face_samples) < self.max_samples:
                self.status_label.setText(f"📸 {samples_text} - Keep your face in frame")
            else:
                self.status_label.setText(f"✅ {samples_text} - Ready to train!")
    ...
    def _stop_and_train(self):
        ...
        if len(self.face_samples) < 10:
            self.status_label.setText(f"❌ Not enough samples ({len(self.face_samples)}). Need at least 10.")
            return
        ...
        self.training_thread = FaceTrainingWorker(self.face_samples, self.user_name)
    ...
    def _on_training_complete(self, success, message):
        ...
        if success:
            self.status_label.setText("✅ Registration complete! You can now use face unlock.")
...
class LoginWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        layout.setContentsMargins(SPACING["xl"], SPACING["3xl"], SPACING["xl"], SPACING["xl"])
        ...
        title.setFont(QFont(FONTS["family"], FONTS["size_3xl"], FONTS["weight_bold"]))
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)
        ...
        # Status label
        self.status_label = QLabel("Click below to login with face recognition")
        self.status_label.setAlignment(Qt.AlignCenter)
        ...
        self.login_btn.setFont(QFont(FONTS["family"], FONTS["size_lg"], FONTS["weight_semibold"]))
        ...
        if os.environ.get('VOCALXPERT_DEBUG', '').lower() == 'true':
            ...
            self.skip_btn.clicked.connect(lambda: self._on_login_success("Guest"))
    ...
    def _check_user_data(self):
        ...
        user_data_path = Path(__file__).parent.parent / "userData" / "userData.pck"
        
        if not model_path.exists() or not user_data_path.exists():
            self.status_label.setText("Welcome! No user registered yet. Please register your face first.")
            self.status_label.setStyleSheet(f"color: {get_theme().warning};")
        else:
            self.status_label.setText("Click below to login with face recognition")
    ...
    def _on_login_finished(self, success: bool, user_name: str):
        ...
        if success:
            ...
            if not model_path.exists():
                ...
                self.status_label.setText("No face data found. Please register first using 'Register Face' below.")
                self.status_label.setStyleSheet(f"color: {get_theme().warning};")
            else:
                self.login_btn.setText("🔓  Try Again")
                self.status_label.setText("Face not recognized. Please try again or register if you're a new user.")
    ...
    def _on_registration_complete(self, success):
        ...
        if success:
            ...
            self.status_label.setText("✅ Registration successful! You can now log in with face recognition.")
            self._reset_ui()
