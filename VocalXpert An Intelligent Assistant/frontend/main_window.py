...
class SafeStreamHandler(logging.StreamHandler):
    """Stream handler that safely encodes Unicode characters for Windows console."""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("vocalxpert.log", encoding="utf-8"),
        SafeStreamHandler(),
    ],
)
...
class Sidebar(QFrame):
    ...
    def _setup_ui(self):
        ...
        layout.setContentsMargins(SPACING["md"], SPACING["lg"], SPACING["md"],
                                  SPACING["lg"])
        ...
        brand.setFont(
            QFont(FONTS["family"], FONTS["size_xl"], FONTS["weight_bold"]))
        ...
        for key, icon, text in nav_items:
            ...
            btn.clicked.connect(
                lambda checked=False, k=key: self._on_nav_click(k))
...
class MainWindow(QMainWindow):
    ...
    def _on_command_result(self, response: str, action_type: str):
        ...
        # Speak response if TTS enabled (always speak in voice mode for
        # feedback)
        settings = self.settings_panel.get_settings()
    ...
    def _on_command_error(self, error: str):
        ...
        self.chat_panel.add_message(f"Sorry, I encountered an error: {error}",
                                    is_user=False)
        self.chat_panel.set_status("ready")
    ...
    def _start_voice_mode(self):
        ...
        self.chat_panel.add_message(
            "🎤 Voice mode activated! Speak naturally and I'll respond automatically.",
            is_user=False,
        )
        ...
        if not SR_AVAILABLE:
            self.chat_panel.add_message(
                "❌ Speech recognition library not available. Please install: pip install SpeechRecognition",
                is_user=False,
            )
            self._stop_voice_mode()
            return
        ...
        if not PYAUDIO_AVAILABLE:
            self.chat_panel.add_message(
                "❌ PyAudio not available. Please install: pip install pyaudio",
                is_user=False,
            )
            self._stop_voice_mode()
            return
        ...
        self._continuous_voice_worker.result_ready.connect(
            self._on_voice_result)
        self._continuous_voice_worker.stopped.connect(
            self._on_voice_mode_stopped)
    ...
    def _on_voice_result(self, text: str):
        ...
        if text and text.strip():
            ...
            self.chat_panel.add_message(
                "Voice recognition returned empty result. Please try speaking more clearly.",
                is_user=False,
            )
    ...
    def _on_voice_error(self, error: str):
        ...
        if error == "timeout":
            # In continuous mode, timeout just means no speech detected - don't
            # show error
            if not self._voice_mode_active:
                self.chat_panel.set_status("ready", "No speech detected")
                self.chat_panel.add_message(
                    "Voice recognition timed out. Please speak louder or closer to the microphone, then try again.",
                    is_user=False,
                )
        elif error == "empty":
            ...
            self.chat_panel.add_message(
                "Network error. Please check your connection.", is_user=False)
        else:
            self.chat_panel.add_message(f"Voice recognition error: {error}",
                                        is_user=False)
    ...
    def _speak(self, text: str):
        ...
        if self._continuous_voice_worker:
            self._tts_worker.finished.connect(
                self._continuous_voice_worker.on_tts_finished)
    ...
    def _cleanup_all_threads(self):
        ...
        try:
            ...
            if (hasattr(self, "_continuous_voice_worker") and
                    self._continuous_voice_worker):
        ...
        try:
            ...
            if hasattr(self, "_tts_worker") and self._tts_worker:
                ...
                if self._tts_worker.isRunning():
                    ...
                    if not self._tts_worker.wait(3000):  # Wait up to 3 seconds
                        logger.warning(
                            "TTS worker didn't stop gracefully, terminating...")
            ...
            if hasattr(self, "_command_worker") and self._command_worker:
                ...
                if self._command_worker.isRunning():
                    # CommandWorker doesn't have a stop method, so just
                    # terminate
                    self._command_worker.terminate()
                    if not self._command_worker.wait(2000):
                        logger.warning(
                            "Command worker didn't terminate gracefully")
...
def create_splash_screen(app: QApplication) -> QSplashScreen:
    ...
    painter.drawText(0, 180, 400, 50, Qt.AlignCenter, "Loading...")
